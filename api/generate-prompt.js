...
function validate(body) {
  const errors = [];
  const { platform, task, subject, vibeLevel, promptMode, customMode } = body;
  if (!platform || typeof platform !== 'string') errors.push('platform required');
  if (!task || typeof task !== 'string') errors.push('task required');
  if (!subject || typeof subject !== 'string' || subject.length > 200) errors.push('subject invalid');
  if (vibeLevel !== undefined && (typeof vibeLevel !== 'number' || vibeLevel < 0 || vibeLevel > 100)) errors.push('vibeLevel invalid');
  if (promptMode && !['quick', 'detailed'].includes(promptMode)) errors.push('promptMode invalid');
  if (customMode && !['rephrase', 'addEnd'].includes(customMode)) errors.push('customMode invalid');
  if (body.topic && body.topic.length > 200) errors.push('topic too long');
  if (body.customInstructions && body.customInstructions.length > 2000) errors.push('customInstructions too long');
  if (body.fileContent && body.fileContent.length > 20000) errors.push('fileContent too long');
  return errors;
}
...
export default async function handler(req, res) {
  ...
  const {
    platform, task, subject, topic, vibeLevel = 50,
    fileContent, toggles, customInstructions,
    promptMode = 'detailed', customMode = 'rephrase',
  } = req.body;

  const GROQ_API_KEY = process.env.GROQ_KEY;
  if (!GROQ_API_KEY) return res.status(500).json({ error: 'SERVER_MISCONFIGURED' });
  ...
  try {
    ...
    if (!response.ok) {
      if (response.status === 401) return res.status(500).json({ error: 'UPSTREAM_KEY_INVALID' });
      if (response.status === 429) return res.status(429).json({ error: 'UPSTREAM_RATE_LIMIT' });
      const err = await response.json().catch(() => ({}));
      return res.status(502).json({ error: err.error?.message || 'UPSTREAM_ERROR' });
    }

    const data = await response.json();
    let generatedPrompt = data.choices[0]?.message?.content?.trim();
    if (!generatedPrompt) return res.status(502).json({ error: 'NO_PROMPT_GENERATED' });
